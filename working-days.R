...
for (i in min(year):max(year)) {		# Páscoa
  ...
  wrkd$hd[easter-2] <- 1			# Sexta-Feira Santa (Paixão de Cristo)
...
wrkd$hd[wrkd$month==1 & wrkd$day==1] <- 1		# Confraternização universal 
wrkd$hd[wrkd$month==5 & wrkd$day==1] <- 1		# Dia do trabalho
wrkd$hd[wrkd$month==4 & wrkd$day==21] <- 1	# Tiradentes
wrkd$hd[wrkd$month==9 & wrkd$day==7] <- 1		# Independência
wrkd$hd[wrkd$month==10 & wrkd$day==12] <- 1	# Nossa Senhora Aparecida
wrkd$hd[wrkd$month==11 & wrkd$day==15] <- 1	# Proclamação da República
